...
struct process_state {
    unsigned int sp_bot; // bottom of the stack for that process//
...
unsigned int process_select (unsigned int cursp) {
    if (current_process == NULL){
        ...
        if (cursp == 0){
            ...
            // Shouldn't be here cuz current_process == NULL should have fired
            digitalWrite(WHITE, 1);
        } 
    ...
    if (current_process == NULL){
        // all processes terminated, or none to begin with
        // waiting processes are hung if they exist
...
__attribute__((used)) void process_terminated ()
{
    ...
    yield(); // -- slightly un-optimized cuz we don't need to save stack of term proc
    // "ldi r24, lo8(0)\n\t" -- error lights would turn on if we did this
    // "ldi r25, hi8(0)\n\t"
    // asm volatile ("rjmp .dead_proc_entry\n\t");
}
